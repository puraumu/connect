...
app.replace = function(route, fn) {
  var len = this.stack.length;
  if (!len) {
    return this;
  };
  this.stack.splice(len - 1);
  return this.use(route, fn);
}

app.handle = function(res, out) {
  var req;
  if (res.hasOwnProperty('req')) req = res.req;
  else if (res.hasOwnProperty('request')) req = res.request;
  else if (res.hasOwnProperty('socket')) req = res.socket._httpMessage;
  else throw new Error('The `res` object seems not to be `clentResponse`.');
